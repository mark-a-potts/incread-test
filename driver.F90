...
!
      ...
      use module_read_netcdf,  only : read_netcdf
!
!-----------------------------------------------------------------------
!
      implicit none
...
!
      ...
      integer              :: rc, grid_id, mype, wrt_mpi_comm
      ...
      grid_id = 1
      call MPI_Init(rc)
      call read_netcdf(trim(filename), &
          ...
          if (trim(output_grid(grid_id)) == 'cubed_sphere_grid') then
            ...
            if (trim(output_file(nnnn)) == 'netcdf_parallel') then
              call write_netcdf(wrt_int_state%wrtFB(nbdl),trim(filename), &
                               ...
                               grid_id,rc)
              write(6,*) 'HEYY!!! calling read_netcdf, with ',trim(filename)
              call read_netcdf(wrt_int_state%wrtFB(nbdl),trim(filename), &
                               .true., wrt_mpi_comm,wrt_int_state%mype, &
                               grid_id,rc)
            else
            ...
            wbeg = MPI_Wtime()
            write(6,*) 'HEYY222!!! calling write_netcdf, with ',trim(filename)
            call write_netcdf(wrt_int_state%wrtFB(nbdl),trim(filename), &
                             use_parallel_netcdf, wrt_mpi_comm,wrt_int_state%mype, &
                             grid_id,rc)
            call read_netcdf(wrt_int_state%wrtFB(nbdl),trim("atminc.nc4"), &
                             use_parallel_netcdf, wrt_mpi_comm,wrt_int_state%mype, &
                             grid_id,rc)
            wend = MPI_Wtime()
            if (lprnt) then
            ...
            wbeg = MPI_Wtime()
            write(6,*) 'HEYY333!!! calling write_netcdf, with ',trim(filename)
            call write_netcdf(wrt_int_state%wrtFB(nbdl),trim(filename), &
                              use_parallel_netcdf, wrt_mpi_comm,wrt_int_state%mype, &
                              grid_id,rc)
            wend = MPI_Wtime()
